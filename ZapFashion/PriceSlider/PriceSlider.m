...
- (CGRect)trackRect
{
    ...
    RectValue.size = CGSizeMake(currentTrackImage.size.width, currentTrackImage.size.height);
    if(currentTrackImage.capInsets.top || currentTrackImage.capInsets.bottom)
    {
        RectValue.size.height=self.bounds.size.height;
    }
    ...
    RectValue.origin = CGPointMake(xLowerValue, (self.bounds.size.height/2.0f) - (RectValue.size.height/2.0f));
    RectValue.size.width = xUpperValue-xLowerValue;
    UIEdgeInsets alignmentInsets = [self trackAlignmentInsets];
    RectValue = UIEdgeInsetsInsetRect(RectValue,alignmentInsets);
    return RectValue;
}
